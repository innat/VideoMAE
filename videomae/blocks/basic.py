...
class TFBlock(keras.Model):
    def __init__(
        ...
        **kwargs,
        ...
        self.drop_path = TFDropPath(drop_path) if drop_path > 0.0 else layers.Identity()
    ...
    def call(self, x, training=None):
        if self.gamma_1 is None:
            ...
            x = x + self.drop_path(
                self.gamma_1 * self.attn(self.norm1(x)), training=training
            )
            x = x + self.drop_path(
                self.gamma_2 * self.mlp(self.norm2(x)), training=training
            )
        return x
